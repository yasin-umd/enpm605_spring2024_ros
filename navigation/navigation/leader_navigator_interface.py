...
class LeaderNavigatorInterface(Node):
    def __init__(self):
        super().__init__(node_name="leader_navigator")
        ...
        self.nav_through_poses_client = ActionClient(
            self, NavigateThroughPoses, "/leader/navigate_through_poses"
        )
        self.nav_to_pose_client = ActionClient(
            self, NavigateToPose, "/leader/navigate_to_pose"
        )
        self.follow_waypoints_client = ActionClient(
            self, FollowWaypoints, "/leader/follow_waypoints"
        )
        self.follow_path_client = ActionClient(self, FollowPath, "/leader/follow_path")
        self.compute_path_to_pose_client = ActionClient(
            self, ComputePathToPose, "/leader/compute_path_to_pose"
        )
        self.compute_path_through_poses_client = ActionClient(
            self, ComputePathThroughPoses, "/leader/compute_path_through_poses"
        )
        self.spin_client = ActionClient(self, Spin, "/leader/spin")
        self.backup_client = ActionClient(self, BackUp, "/leader/backup")
        ...
        self.initial_pose_pub = self.create_publisher(
            PoseWithCovarianceStamped, "leader/initialpose", 10
        )
        self.change_maps_srv = self.create_client(LoadMap, "/leader/map_server/load_map")
        self.clear_costmap_global_srv = self.create_client(
            ClearEntireCostmap, "/leader/global_costmap/clear_entirely_global_costmap"
        )
        self.clear_costmap_local_srv = self.create_client(
            ClearEntireCostmap, "/leader/local_costmap/clear_entirely_local_costmap"
        )
        self.get_costmap_global_srv = self.create_client(
            GetCostmap, "/leader/global_costmap/get_costmap"
        )
        self.get_costmap_local_srv = self.create_client(
            GetCostmap, "/leader/local_costmap/get_costmap"
        )
    ...
    def _waitForNodeToActivate(self, node_name):
        ...
        node_service = f"/leader/{node_name}/get_state"
